<template>
  <div class="home-top">
    <nav-bar>
      <template #left>
        <van-icon name="like" :dot="showdot" @click="showHomePopup" />
        <!-- @click="openMenu" -->
      </template>
      <template #title>
    ...
    <home-tab @getHomeTabName1="showHomeTabName1"></home-tab>
    <!-- <drawer></drawer> -->
    <!-- 写成组件有bug -->
    <van-popup
      v-model="showhomedrawer"
...
import HomeTab from "components/content/tab/home/HomeTab";
...
export default {
  ...
  methods: {
    showHomePopup() {
      ...
      // this.showhomedrawer = "home";
      // setTimeout(() => {
      //   this.$root.bus.$emit("showDrawer", this.showhomedrawer);
      // }, 20);
    },
    Overlay() {
    ...
    showHomeTabName1(name) {
      //左name是data内的name，右name是HomeTab中van-tab的name
      this.name = name;
      this.$emit("getHomeTabName2", this.name);
    },
    //跳转WebSearch组件
    search() {
